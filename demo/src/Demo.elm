...
url : String
url =
    "https://elmfire.firebaseio-demo.com/demo"
...
progression note model =
    ...
    , tasks =
        case note of
            LogTaskStart step ->
                replaceOrAppend step note model.tasks

            LogTaskSuccess step _ ->
                replaceOrAppend step note model.tasks

            LogTaskFailure step _ ->
                replaceOrAppend step note model.tasks
    ...
    case tasks of
        ...
        ( s1, n1 ) :: rest ->
            if s1 == step then
                ...
                ( s1, n1 ) :: replaceOrAppend step note rest


state : Signal Model
state =
    Signal.foldp progression startModel logEntries
        ...
        (\( step, logEntry ) ->
            div [ class "line" ]
                ...
                , case logEntry of
                    ...
                    LogTaskSuccess _ res ->
                        span [ class "success" ] [ text res ]

                    LogTaskFailure _ err ->
                        span [ class "failure" ] [ text err ]
...
doSleep id seconds =
    let
        step =
            "sleep " ++ id ++ " for " ++ (toString seconds) ++ " seconds"
    in
        Signal.send notes.address (LogTaskStart step)
            `andThen`
                \_ ->
...
port runTasks =
    ...
    in
        doSubscribe "query1 value" (valueChanged noOrder) loc
            ...
            `andAnyway`
                doSubscribe "query2 parent value"
            ...
            `andAnyway`
                (doSubscribe "subscribe" (valueChanged noOrder) loc
            ...
            `andAnyway`
                doUpdate "update object a and d"
            ...
            `andAnyway`
                (open (loc |> sub "e" |> push)
                    `andThen`
                        \ref ->
                            doSet "push set" (JE.string <| key ref) (location ref)
                                `andThen` \ref -> doSetPriority "setPriority" (NumberPriority 17) (location ref)
                )
            `andAnyway` succeed ()

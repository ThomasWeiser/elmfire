...
state : Signal Model
state =
    Signal.foldp progression startModel reports.signal
...
type alias TestTask x a =
    Context -> Task x a


runTest : TestTask x a -> Task x a
runTest testTask =
    testTask "no test name"
...
createReporter description =
    \context ->
        report context RunReporter description
            `andThen` \_ -> succeed (\val -> report context TestPrint (toString val))
            ...
            `andThen`
                \val ->
                    ((if condition val then
                        report context TestPass description
                      else
                        report context TestError (description ++ " [unfulfilled]")
                     )
                        `andThen` \_ -> succeed val
                    )
